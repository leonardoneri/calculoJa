...
export const metadata: Metadata = {
  title: "Calculadoras Veterinárias - Calculadoras Especializadas",
  description: "Ferramentas especializadas para cuidados com animais de estimação e cálculos veterinários.",
}

export default async function VeterinaryCategoryPage() {
  let calculators = []
  let errorOccurred = false
  
  try {
    calculators = await getCalculatorsByCategory(CalculatorCategory.VETERINARY)
  } catch (error) {
    console.error("Erro ao buscar calculadoras veterinárias:", error)
    errorOccurred = true
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold mb-4">Calculadoras Veterinárias</h1>
        <p className="text-lg text-gray-600">
          Ferramentas especializadas para cuidados com animais de estimação.
        </p>
      </div>
      ...
      {errorOccurred ? (
        ...
        <div className="text-center py-8 bg-yellow-50 rounded-lg border border-yellow-100">
          ...
          <p className="text-gray-700 mb-4">
            Ainda não temos calculadoras disponíveis nesta categoria, mas estamos desenvolvendo novas ferramentas veterinárias para você!
          </p>
          <Link href="/" className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-lg">
            Voltar para a página inicial
          </Link>
